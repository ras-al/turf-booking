...
function TurfListCard({ turf }: { turf: Turf }) {
  return (
    ...
    >
      <Link
        href={`/turfs/${turf.id}`}
        className="block glass-card overflow-hidden hover:border-turf/30 transition-all group hover:glow-accent"
      >
        <div className="h-40 bg-gradient-to-br from-pitch-700/80 to-pitch-800/80 relative overflow-hidden grain-overlay">
          <div className="absolute inset-0 flex items-center justify-center group-hover:scale-110 transition-all duration-500">
            {SPORT_ICONS[turf.sports[0]] ? (
              <div className="w-16 h-16 [&>svg]:w-full [&>svg]:h-full opacity-30 text-pitch-500 group-hover:text-turf/20">
                {SPORT_ICONS[turf.sports[0]]}
              </div>
            ) : (
          ...
          <div className="absolute top-3 left-3 flex gap-1.5 z-10">
            {turf.sports.slice(0, 2).map((sport) => (
              <span
                key={sport}
                className="px-2 py-1 text-xs font-semibold bg-pitch-900/60 border border-turf/20 text-turf rounded-md backdrop-blur-md capitalize flex items-center gap-1"
              >
          ...
          {turf.size && (
            <span className="absolute top-3 right-3 px-2 py-1 text-xs font-mono font-bold bg-amber/90 text-pitch-900 rounded-md z-10 shadow-lg">
              {turf.size}
            </span>
          )}
        ...
        <div className="p-5">
          ...
          <p className="text-sm text-chalk-dim mt-1 flex items-center gap-1.5 truncate">
            <MapPin className="w-3.5 h-3.5 shrink-0" />
            {turf.address}
          </p>

          <div className="mt-3 flex items-center justify-between">
            <span className="text-lg font-mono font-bold text-turf">
              {formatCurrency(turf.price_per_hour)}
              <span className="text-xs text-chalk-dim font-body">/hr</span>
            </span>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {turf.amenities.slice(0, 4).map((a) => (
              <span key={a} className="px-2 py-0.5 text-xs bg-pitch-700 text-chalk-dim rounded-md capitalize">
                {a}
              </span>
            ))}
            {turf.amenities.length > 4 && (
              <span className="px-2 py-0.5 text-xs bg-pitch-700 text-chalk-dim rounded-md">
                +{turf.amenities.length - 4}
              </span>
            )}
          </div>
...
function TurfsContent() {
  ...
  const [turfsWithDistance, setTurfsWithDistance] = useState<Turf[]>(MOCK_TURFS);
  ...
  useEffect(() => {
    if (!userLocation) {
      setTurfsWithDistance(MOCK_TURFS);
      return;
    }

    async function calcDistances() {
      const results = await Promise.all(
        MOCK_TURFS.map(async (turf) => {
  ...
  const filteredTurfs = useMemo(() => {
    ...
    if (filters.minPrice !== undefined) {
      result = result.filter((t) => t.price_per_hour >= filters.minPrice! * 100);
    }
    if (filters.maxPrice !== undefined) {
      result = result.filter((t) => t.price_per_hour <= filters.maxPrice! * 100);
    }
    if (filters.size) {
      result = result.filter((t) => t.size === filters.size);
    }
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-medium rounded-lg border transition-all flex items-center gap-1.5 ${
                  isFilterOpen
                    ? 'bg-turf/10 border-turf/30 text-turf'
                    : 'bg-pitch-800 border-pitch-600/50 text-chalk-muted hover:text-chalk'
                }`}
              >
                ...
                Filters
                {activeFilterCount > 0 && (
                  <span className="w-5 h-5 rounded-full bg-turf text-pitch-900 text-xs font-bold flex items-center justify-center">
                    {activeFilterCount}
                  </span>
                )}
              </button>
          ...
          <div className="mt-4">
            <div className="relative max-w-md">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-chalk-dim" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-pitch-800 border border-pitch-600/50 text-chalk text-sm rounded-lg placeholder-chalk-dim focus:outline-none focus:border-turf/50"
              />
            </div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {isFilterOpen && (
          <motion.div
            ...
            className="bg-pitch-800 border-b border-pitch-600/30 overflow-hidden"
          >
            <div className="max-w-7xl mx-auto px-4 py-5">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={filters.sport || ''}
                    onChange={(e) => setFilter('sport', e.target.value || undefined)}
                    className="w-full px-3 py-2 bg-pitch-700 border border-pitch-600 text-chalk text-sm rounded-lg focus:outline-none focus:border-turf/50 capitalize"
                  >
                ...
                <div>
                  ...
                  <select
                    value={filters.size || ''}
                    onChange={(e) => setFilter('size', e.target.value || undefined)}
                    className="w-full px-3 py-2 bg-pitch-700 border border-pitch-600 text-chalk text-sm rounded-lg focus:outline-none focus:border-turf/50"
                  >
                ...
                <div className="flex items-end">
                  <button
                    ...
                    className="w-full px-3 py-2 text-sm text-danger border border-danger/30 rounded-lg hover:bg-danger/10 transition-colors"
                  >
                    Clear All
                  </button>
                </div>
        ...
        {filteredTurfs.length === 0 ? (
          <div className="text-center py-20">
            <div className="flex justify-center mb-6">
              <Search className="w-16 h-16 text-chalk-dim/50" />
            </div>
            ...
            <button
              onClick={resetFilters}
              className="mt-4 px-5 py-2 text-sm bg-turf text-pitch-900 rounded-lg font-medium hover:bg-turf-light transition-colors"
            >
              Reset Filters
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTurfs.map((turf) => (
              <TurfListCard key={turf.id} turf={turf} />
            ))}
          </div>
        )}
      </div>
