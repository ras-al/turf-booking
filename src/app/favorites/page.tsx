...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    fetchTurfs().then((data) => { setTurfs(data); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="md:hidden pt-16 pb-[84px]">
        <div className="px-4 pb-3"><h1 className="text-xl font-bold text-gray-900">Favorites</h1></div>
        <div className="px-4">
          {loading ? (
            <div className="space-y-3 py-4">{[1, 2, 3].map(i => (<div key={i} className="flex gap-3 animate-pulse"><div className="w-[68px] h-[68px] rounded-xl bg-gray-100" /><div className="flex-1 space-y-2 py-1"><div className="h-4 bg-gray-100 rounded w-3/4" /><div className="h-3 bg-gray-50 rounded w-1/2" /><div className="h-3 bg-gray-50 rounded w-1/4" /></div></div>))}</div>
          ) : turfs.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-sm text-gray-400 mt-1">Save turfs you love for quick access</p>
              <Link href="/turfs" className="mt-4 inline-flex px-5 py-2.5 bg-green-600 text-white rounded-xl font-bold text-sm">Explore Turfs</Link>
            </div>
          ) : (
            <div>
              {turfs.map((turf) => (
                <Link key={turf.id} href={`/turfs/${turf.id}`} className="flex items-start gap-3 py-3 border-b border-gray-100 last:border-0">
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    ...
                    <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3 text-green-600 shrink-0" /><span className="truncate">{turf.address}</span></p>
                    <div className="flex items-center gap-1 mt-1"><Star className="w-3.5 h-3.5 text-amber fill-amber" /><span className="text-xs font-bold text-gray-800">{turf.avg_rating}</span><span className="text-xs text-gray-400">({turf.total_reviews})</span></div>
                  </div>
                  <div className="flex flex-col items-end pt-1"><Heart className="w-4 h-4 text-red-500 fill-red-500" /></div>
                </Link>
              ))}
            </div>
          )}
      ...
      <div className="hidden md:block py-8 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          {loading ? (
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">{[1, 2, 3].map(i => <div key={i} className="h-64 bg-gray-100 rounded-2xl animate-pulse" />)}</div>
          ) : turfs.length === 0 ? (
                    ...
                    <div className="h-36 bg-gray-100 relative overflow-hidden">
                      {turf.photos && turf.photos.length > 0 ? (
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-20">
                          {SPORT_ICONS[turf.sports[0]] ? <div className="w-20 h-20 [&>svg]:w-full [&>svg]:h-full">{SPORT_ICONS[turf.sports[0]]}</div> : <MapPin className="w-20 h-20" />}
                        </div>
                      )}
                    </div>
                    <div className="p-4">
